...
// Enhanced type for the analytics data
type CompanyAnalytics = Company & {
    plan: { name: MultilingualString } | null;
    _count: { jobs: number };
};

// --- Reusable Form Components from original page (no changes needed) ---
const CompanyForm: React.FC<{ company: Partial<Company>, onSave: (data: any) => void, onCancel: () => void }> = ({ company, onSave, onCancel }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <MultilingualInput label={t('admin.companies.form.name')} name="name" value={formData.name || { es: '', en: '', fr: '' }} onChange={(value) => handleMultilingualChange('name', value)} />
            <div>
                <label className="block text-sm font-medium">{t('admin.companies.form.logo')}</label>
                <ImageUploader initialValue={formData.logo || ''} onValueChange={handleImageChange} />
            </div>
            <MultilingualTextarea label={t('admin.companies.form.description')} name="description" value={formData.description || { es: '', en: '', fr: '' }} onChange={(value) => handleMultilingualChange('description', value)} rows={4} />
            ...
            <div className="pt-2"><SocialLinksManager value={formData.socialLinks || []} onChange={handleSocialLinksChange} /></div>
            <div className="flex items-center pt-2 border-t"><input type="checkbox" id="isRecruitmentClient" name="isRecruitmentClient" checked={!!formData.isRecruitmentClient} onChange={handleInputChange} className="h-4 w-4 text-primary border-gray-300 rounded focus:ring-primary" /><label htmlFor="isRecruitmentClient" className="ml-2 block text-sm font-medium text-gray-900">{t('admin.companies.form.isRecruitmentClient')}</label></div>
            <div className="flex justify-end space-x-3 pt-4 border-t mt-4"><Button type="button" variant="light" onClick={onCancel}>{t('admin.companies.form.cancel')}</Button><Button type="submit">{t('admin.companies.form.save')}</Button></div>
        </form>
    );
};
    ...
    return (
        <div>
            <div className="mb-4">
                ...
                <select id="plan-select" value={selectedPlanId} onChange={(e) => setSelectedPlanId(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md bg-white text-gray-900">
                    <option value="">{t('admin.companies.changePlanModal.noPlan')}</option>
                    {plans.map(plan => (<option key={plan.id} value={plan.id}>{t_dynamic(plan.name)}</option>))}
                </select>
            </div>
            <div className="flex justify-end space-x-3"><Button variant="light" onClick={onCancel}>{t('admin.companies.changePlanModal.cancel')}</Button><Button onClick={() => onSave(selectedPlanId)}>{t('admin.companies.changePlanModal.save')}</Button></div>
        </div>
    );
};

// --- New Analytics & Filter Components ---
...
const FilterBar: React.FC<{ onFilterChange: (value: string) => void }> = ({ onFilterChange }) => {
    const { t } = useI18n();
    ...
    return (
        <form onSubmit={handleFilter} className="mb-6 p-4 bg-gray-50 rounded-lg flex items-center gap-4">
            <input
                type="text"
                placeholder={t('admin.companies.filterByLocation')}
                value={location}
                onChange={(e) => setLocation(e.target.value)}
                className="flex-grow p-2 border rounded-md bg-white text-gray-900"
            />
            <Button type="submit">{t('admin.companies.filter')}</Button>
        </form>
    );
};
...
const AdminCompaniesPage: React.FC = () => {
    ...
    const fetchCompaniesAndPlans = useCallback(async () => {
        ...
        try {
            const [companiesData, plansData] = await Promise.all([
                api.getCompaniesAnalytics({ location: locationFilter }),
    ...
    const analytics = useMemo(() => {
        ...
        const planCounts = companies.reduce((acc, c) => {
            const planName = c.plan ? t_dynamic(c.plan.name) : t('admin.companies.noPlan');
            acc[planName] = (acc[planName] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        return { totalCompanies, totalJobs, planCounts };
    }, [companies, t, t_dynamic]);
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-semibold text-gray-800">{t('admin.companies.dashboardTitle')}</h1>
                <Button onClick={handleCreateNew}>{t('admin.companies.add')}</Button>
            </div>

            {/* KPI Section */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <KpiCard title={t('admin.companies.kpi.totalCompanies')} value={analytics.totalCompanies} />
                <KpiCard title={t('admin.companies.kpi.totalJobs')} value={analytics.totalJobs} />
                <KpiCard title={t('admin.companies.kpi.proCompanies')} value={analytics.planCounts['Pro Mensual'] || 0} />
            </div>

            <FilterBar onFilterChange={setLocationFilter} />
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="p-4 text-sm font-semibold text-gray-600">{t('admin.companies.registrationDate')}</th>
                                    <th className="p-4 text-sm font-semibold text-gray-600">{t('admin.companies.plan')}</th>
                                    <th className="p-4 text-sm font-semibold text-gray-600">{t('admin.companies.jobPosts')}</th>
                                    <th className="p-4 text-sm font-semibold text-gray-600">{t('admin.companies.location')}</th>
                                ...
                                {companies.map(company => (
                                    <tr key={company.id} className="hover:bg-gray-50">
                                        <td className="p-4 font-medium flex items-center gap-3">
                                            <img src={company.logo || ''} alt={t_dynamic(company.name)} className="w-10 h-10 rounded-md object-cover" />
                                            {t_dynamic(company.name)}
                                        </td>
                                        <td className="p-4 text-gray-600">{formatDate(company.createdAt)}</td>
                                        <td className="p-4 text-gray-600">{company.plan ? t_dynamic(company.plan.name) : t('admin.companies.noPlan')}</td>
                                        <td className="p-4 text-gray-600 text-center">{company._count.jobs}</td>
                                        <td className="p-4 text-gray-600 truncate max-w-xs">{company.address}</td>
                                        <td className="p-4">
